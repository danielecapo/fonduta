...
(defn ascender [f]
  (get (get f :alignments) :ascender))

(defn descender [f]
  (get (get f :alignments) :descender))
...
(defn draw-glyphs [f zero & glyphs]
  (let [gs (remove nil? (map (fn [g] (get-glyph f g)) glyphs))
        advances (reverse (reduce (fn [l g] (cons (+ (first (:advance g)) (first l)) l))
                                  (list zero)
                                  gs))
        ascender (ascender f)]
    (fn [c g]
...
;;(draw-glyphs f 200  0 :o :i :o))))))
                                                  ;;(fn [c g]
                                                    ;;       (do
                                                      ;;       (draw-glyph g (get-glyph f :o) 600 (ascender f) 0)
                                                        ;;     (draw-glyph g (get-glyph f :o) 600 (ascender f) 600))))))))
                                                          ;;f 600 0 :i :i :i :o :o :o :i :i :i))))))
;;                                                  (get-glyph f :o) 600 (ascender f) 0))))))
